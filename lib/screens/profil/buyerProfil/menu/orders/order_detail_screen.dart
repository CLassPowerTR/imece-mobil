...
class OrderDetailScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final String siparisId =
        (item['siparis_id'] ?? item['id'] ?? '-').toString();
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: customText('Sipariş Detayları', context,
                    ...
                    child: Row(
                      ...
                      children: [
                        _MetricTile(
                          ...
                          label: 'Toplam tutar',
                          valueStyle:
                              AppTextStyle.bodyLargeBold(context).copyWith(
          ...
          children: [
            Expanded(
              ...
              child: Row(
                ...
                spacing: 16,
                children: [
                  customText('${urun['urun_bilgileri']['urun_adi']}', context,
                      style: AppTextStyle.bodyMediumBold(context),
                      maxLines: 2,
                      overflow: TextOverflow.ellipsis),
                  customText('${urun['urun_bilgileri']['aciklama']}', context,
                      style: AppTextStyle.bodySmallMuted(context),
                      maxLines: 3,
                      overflow: TextOverflow.ellipsis),
                  customText('Miktar: ${urun['miktar']}', context,
                      maxLines: 2, style: AppTextStyle.bodySmallMuted(context)),
                ],
              ),
            ),
            Expanded(
              ...
              child: Column(
                ...
                children: [
                  customText(
                    ...
                    context,
                    maxLines: 2,
                    style: AppTextStyle.bodyLargeBold(context).copyWith(
                      color: AppColors.secondary(context),
                    ),
                  ),
                  TextButton.icon(
                      ...
                      onPressed: () async {
                        ...
                        try {
                          final String id =
                              urun['urun_bilgileri']['urun_id'].toString();
                          final Product product =
                              await ApiService.fetchProduct(int.parse(id));
                          ...
                          if (product == null) {
                            showTemporarySnackBar(context, 'Ürün bulunamadı',
                                type: SnackBarType.info);
                          }
                        } catch (_) {
                          showTemporarySnackBar(context, 'Hata: $_',
                              type: SnackBarType.error);
                          Navigator.of(context).pop();
                        }
                      },
                      label: customText('İncele', context,
                          maxLines: 1,
                          style: AppTextStyle.bodySmallMuted(context)))
                ],
              ),
            ...
            RichText(
              ...
              text: TextSpan(
                children: [
                  ...
                  TextSpan(
                      text: title, style: AppTextStyle.bodyLargeBold(context)),
                ],
              ),
            ),
            if (value != null)
              customText(value, context,
                  maxLines: 5, style: AppTextStyle.bodyMedium(context)),
            if (item != null) item,
          ],
        ));
  }
...
class _MetricTile extends StatelessWidget {
  ...
  const _MetricTile(
      {required this.value, required this.label, this.valueStyle});
